<!-- README
    This is an updated version using James's Spotify app to Auth. 
    - Auth page https://spotify-demo-auth.glitch.me/
    - Only need accessToken and refreshToken keys in Custom Tile.
    - Changed a few aspects from the original code to improve layout and design.  
-->
<!-- Do not edit below -->
...
#lists .active {background-color:#3C0125!important; color:##D0D0D6!important; border:unset;}
...
.list-group li:hover {
  ...
  background-color:#3C0125!important;
  opacity: 0.5; /* Change this value to adjust transparency */
}
...
/*function updateSpotifyMainBackground(imageUrl) {
  ...
  spotifyMain.style.backgroundImage = `url(${imageUrl})`;
}*/
      ...
      headers: {"Authorization": "Bearer " + userSettings.token}
        ...
        if(err.response && isTokenExpiredError(err.response)){
          ...
          if (err.response.status === 401){
            showError("Unauthorized. Please review the token setting and try it agian.");
          }
          reject(err);
        }
      });
  ...
  axios({method, url, data,    
    headers: {"Authorization": "Bearer " + userSettings.token}
      ...
      if(err.response && isTokenExpiredError(err.response)){
        ...
        showError("No active device. Please select an availabie device first.");          
      }
